...
const UsernameInput = ({
    ...
    const getStatusIcon = () => {
        if (isChecking) {
            return <FiLoader className="animate-spin text-white/50" size={16} />;
        }
        if (isAvailable === true) {
    ...
    return (
        <div className={`${className}`}>
            ...
            <div className="flex gap-2">
                <div className="relative flex-1">
                    ...
                    <input
                        ...
                        className={`w-full pl-9 pr-10 py-3 bg-white/10 border rounded-xl text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all ${error ? 'border-red-400/50' : isAvailable ? 'border-green-400/50' : 'border-white/20'
                            }`}
                    />
            ...
            {currentUsername && !username && (
                <p className="text-white/50 text-xs mt-1">Current: @{currentUsername}</p>
            )}
        </div>
    );
};
