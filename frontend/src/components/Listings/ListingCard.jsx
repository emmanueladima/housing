      ...
      <div className="p-5">
        ...
        <div className="flex items-center gap-2 mb-3 flex-wrap">
          ...
          {listing.averageRating > 0 && (
            <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-bold bg-yellow-50 text-yellow-700 border border-yellow-200">
              <svg className="w-3 h-3 fill-yellow-400 text-yellow-400 mr-1" viewBox="0 0 24 24">
                <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
              </svg>
              {listing.averageRating.toFixed(1)}
              {listing.reviews?.length > 0 && (
                <span className="text-yellow-600 font-normal ml-1">({listing.reviews.length})</span>
              )}
            </span>
          )}
